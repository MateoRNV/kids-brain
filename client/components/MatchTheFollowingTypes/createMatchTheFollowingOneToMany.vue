    ...
    <v-form ref="form2" v-if="screen == 2" class="d-flex flex-column align-center screen-2">
        ...
        <div :class="['text-input ans-section', (!form2Valid && (Object.keys(ansSet).length < 1 || Object.keys(ansValues).length < Object.keys(ansSet).length) && 'error-border')]" >
          ...
          <ul v-if="ansType == 'Text Answer'" class="text-type-ans-set">
            ...
            <span v-if="ansSet.length < 10" class="addNewAns" @click="() => addOptionForMultiple(null,'text')">Add Answer <v-icon>mdi-plus-circle</v-icon></span>
          </ul>
          <ul v-if="ansType == 'Image Answer'" class="image-type-ans-set">
            ...
            <span v-if="ansSet.length < 10" class="addNewAns" @click="() => addOptionForMultiple(null, 'image')">Add Answer <v-icon>mdi-cloud-upload</v-icon></span>
          </ul>
              ...
              <ul class="select">
                  <li v-for="(ques,idx) in Object.keys(quesValues)" :key="idx">
                    <div class="grid-2 align-center">
                      <div  class="label">
                        <span v-if="quesValues[ques]">{{ques}}</span>&nbsp;{{quesValues[ques]}}
                      </div>
                      <div class="value">
                         <v-select
...
export default {
  name: 'CreateMatchTheFollowingOneToMany',
  ...
  methods:{
    ...
    async goToNext(type = null){
      ...
      if(this.questionData && this.screen == 1){
          this.ansSet = Object.keys(this.ansValues).map(x=>{
              return Object.values(this.ansKeys).find(y => y.key == x);
          });
      }
      if(this.screen == 3){
    ...
    addOptionForMultiple(isQuestions = null, type = 'text'){
      let n =  this.ansSet.length + 1;
      let i = 1;
      if(this.ansSet.length < 25){
        while (this.ansSet.length < n) {
          const key = this.ansKeys[i];
          let index = this.ansSet.findIndex(x=>{
              return x.key == key.key;
          })
          if( index < 0 && !isQuestions){
            this.ansSet = [...this.ansSet,key];
            this.ansValues[key.key] = null; 
          }
          if( index < 0 && isQuestions){
    ...
    async submit(type){
        let data = {
            ...
            status: type,
            type_id: 4, // for MTFTOTM question type id will be 4
            points: this.points,
            set_id:this.setId,
